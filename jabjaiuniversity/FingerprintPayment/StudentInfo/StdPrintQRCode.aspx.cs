...
{
    public partial class StdPrintQRCode : StudentGateway
    {
                    ...
                    if (sID == 0)
                    {
                        ...
                        switch (userType)
                        {
                            case "0":
                                ...
                                query = string.Format(@"
SELECT u.sID, u.sName 'Name', u.sLastname 'Lastname', ISNULL(tt.titleDescription, u.sStudentTitle) 'Title'
FROM TUser u 
...
", schoolID, sqlCondition);
                            ...
                            case "1":
                                ...
                                query = string.Format(@"
SELECT e.sEmp 'sID', e.sName 'Name', e.sLastname 'Lastname', ISNULL(tt.titleDescription, e.sTitle) 'Title'
FROM TEmployees e 
...
", schoolID, sqlCondition);
                        ...
                        foreach (var i in result)
                        {
                            ...
                            ltrPages.Text += DocumentFormat(schoolName, studentName, qrCodeBase64);
                        }
                    }
                    else
                    {
                        // Print One
                        var user = dbMaster.TUsers.Where(w => w.nSystemID == sID && w.cType == userType && w.cDel == null && w.nCompany == company.nCompany).FirstOrDefault();
                        if (user != null)
                        {
                            string code = string.Format(@"{0}-{1}", company.nCompany, user.sID);
                            string url = string.Format(@"line://app/{0}/?ssid={1}", liffID, EncryptionHelper.Encrypt(code));
                            ...
                            TTitleList title = new TTitleList();

                            switch (userType) {
                                case "0":
                                    var student = dbSchool.TUser.Where(w => w.sID == sID && w.cType == "0" && w.cDel == null).FirstOrDefault();
                                    title = dbSchool.TTitleLists.Where(w => w.nTitleid.ToString() == student.sStudentTitle).FirstOrDefault();
                                    studentName = (title == null ? "" : title.titleDescription) + "" + (student == null ? "" : student.sName + " " + student.sLastname);
                                    break;
                                case "1":
                                    var employee = dbSchool.TEmployees.Where(w => w.sEmp == sID && w.cDel == null).FirstOrDefault();
                                    title = dbSchool.TTitleLists.Where(w => w.nTitleid.ToString() == employee.sTitle).FirstOrDefault();
                                    studentName = (title == null ? "" : title.titleDescription) + "" + (employee == null ? "" : employee.sName + " " + employee.sLastname);
                                    break;
                            }
                        }
        ...
        private string DocumentFormat(string schoolName, string studentName, string qrCodeBase64)
        {
...
<div class=""page"">
    <div class=""subpage"">
        ...
        <div class=""text-center"" style=""width: 270px; float: left; text-align: left; margin-left: 10px; margin-top: 5px; font-weight: bold;"">
            <span class=""header-title"">{0}</span><br />
            <span class=""header-title"">ชื่อ {1}</span><br />
        ...
        <div class=""no-border tab-2"" style=""padding: 30px 0 0 0;"">
            <table class=""table-fixed"">
                ...
                <tr>
                    <td class=""text-label text-left"" style=""padding-left: 10px; text-decoration: underline; font-weight: bold;"">วิธีลงทะเบียนระบบไลน์เพื่อรับข่าวสาร</td>
                </tr>
                <tr>
                ...
                <tr>
                    <td class=""text-label text-center"">
                        <img src=""{2}"" width=""200"" class=""picture-src"" title=""""></td>
                </tr>
                <tr>
...
</div>", schoolName, studentName, qrCodeBase64);
