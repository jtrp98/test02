...
{
    public partial class RegisterOnline09 : RegisterGateway
    {
        ...
        private void InitialControl()
        {
            ...
            using (JabJaiEntities en = new JabJaiEntities(Connection.StringConnectionSchool(schoolID,ConnectionDB.Read)))
            {
                ...
                if (result.Count > 0)
                {
                    ...
                    {
                        this.ltrParentTitle.Text += string.Format(@"<option value=""{0}"">{1}</option>", r.nTitleid, r.titleDescription);
                    }
                }
                ...
                {
                    this.ltrParentNation.Text += string.Format(@"<option value=""{0}"">{1}</option>", l.MasterCode, l.MasterDes);
                }
                ...
                {
                    this.ltrParentReligion.Text += string.Format(@"<option value=""{0}"">{1}</option>", l.MasterCode, l.MasterDes);
                }
                ...
                {
                    this.ltrParentRace.Text += string.Format(@"<option value=""{0}"">{1}</option>", l.MasterCode, l.MasterDes);
                }

                //Script Required Field
                RegisterOnline registerOnline = (RegisterOnline)this.Master;
                List<string> listElement = registerOnline.GetElementRequiredField(schoolID, 6);
