...
{
    public partial class perRegisterCreate : Page
    {
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["InvoiceId"]) || !string.IsNullOrEmpty(Request.QueryString["Mode"]))
            {
                ...
                if (!string.IsNullOrEmpty(Request.QueryString["InvoiceId"]))
                {
                    ...
                    if (Model != null)
                    {
                        ...
                        Model.PaidPayments.ForEach(f => f.Payments = f.Payments.Where(r => r.isDel != true).ToList());
                        ...
                        using (JabJaiMasterEntities jabJaiMasterEntities = Connection.MasterEntities(ConnectionDB.Read))
                        {
                            ...
                            {
                                var f_studet = entities.TPreRegisters.FirstOrDefault(f => f.preRegisterId == Model.preRegisterId);
                                var q_titles = entities.TTitleLists.Where(w => w.SchoolID == userData.CompanyID).ToList();
                                Model.StudentName = getTitlte(q_titles, f_studet.StudentTitle.ToString()) + " " + f_studet.sName + " " + f_studet.sLastname;
                            }
                        }
                    }
                }
