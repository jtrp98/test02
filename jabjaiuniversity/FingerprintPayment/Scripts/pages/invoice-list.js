...
var _currentPage = function () {
    var initPage = function () {
        ...
        $("input[type='radio'][name='rdPaymentType']").change(function () {
            $("#ul-paymentmethod li[data-paymentmethod][data-paymentmethod!='" + $(this).val() + "']").addClass("d-none");
            $("#ul-paymentmethod li[data-paymentmethod][data-paymentmethod='" + $(this).val() + "']").removeClass("d-none");
        });
        ...
        $("#ddl-search-by").change(function () {
            $("#ddl-term,#dateRange").addClass('d-none');
            if ($(this).val() === "term")
                $("#ddl-term").removeClass('d-none');
...
var fnDataTable = function () {
    var initTableSearch = function (tableId) {
        ...
        oTable.dataTable({
            ...
            "ajax": {
                ...
                "beforeSend": function () {
                    ...
                    $.blockUI({ message: '<h3>กำลังดำเนินการกรุณารอสักครู่</h3>' });
                },
                'data': function (data) {
                    if ($("#invoiceid").val() !== "")
                        data.columns[1].search.value = $('#invoiceid').val();
                    if (SAC.GetStudentName() !== "")
                        data.columns[5].search.value = SAC.GetStudentName();
                    //if ($("#studentcode").val() !== "")
                    ...
                    data.columns[19].search.value = "invoice";
            ...
            "aoColumns": [
                ...
                { "mData": "Term", "sName": "Term", "orderable": false },
                { "mData": "StudentCode", "sName": "StudentCode", "aTargets": [1] },
                {
                    "mData": "Date", "sName": "Date", "orderable": false,
                    render: function (data, type, row, meta) {
                        return (row.Fd_NewTermSubLevel ?? row.SubLevel) + ' / ' + (row.Fd_NewTermSubLevel2 ?? row.SubLevel2);
                    }
                },
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        //if (row.TotalAmount <= row.ManualDiscount) {
                        //    let OutstandingAmount = 0;
                        //    OutstandingAmount = row.OutstandingAmount - row.ManualDiscount;
                        //    return OutstandingAmount.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                        //}
                        //else 
                        if (row.GrandTotal !== null) {
                            let GrandTotal = 0;
                            $.each(row.InvoiceItems, function (e, s) {
                                if (s.IsDelete == false) {
                                    GrandTotal += s.Amount - s.Discount;
                                }
                            });

                            GrandTotal -= row.ManualDiscount;

                            return GrandTotal.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                        }
                        else
                            return "0.00";
                    }
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        ...
                        else if (row.OutstandingAmount !== null)
                ...
                { "mData": "InvoiceId", "sName": "SubLevelId", "aTargets": [12], "bVisible": false },
                {
                    ...
                    render: function (data, type, row, meta) {
                        ...
                        if (row.InvoiceStatus === invoiceApproveStatus) {
                            content += '<li><a target="_blank" style=\"\" data-id="' + row.InvoiceId + '" data-code="' + row.InvoiceCode + '"  href="/Modules/Invoices/PrintInvoicePreview.aspx?InvoiceId=' + row.InvoiceId + '">ใบแจ้งหนี้</a></li>';
                            //content += '<li><a href=\"#\" style=\"\" data-id="' + row.InvoiceId + '" data-code="' + row.InvoiceCode + '" data-number="' + row.Code + '" onclick="PopupInvoicePayment(this)">รับชำระ</a></li>';
                            btnPayment += '&nbsp;<a href=\"#\" style=\"\" class="btn btn-primary " data-id="' + row.InvoiceId + '" data-code="' + row.InvoiceCode + '" data-number="' + row.Code + '" onclick="PopupInvoicePayment(this)">รับชำระ</a>';
                        }
                        ...
                        if (row.OutstandingAmount != 0)
                            content += '<li><a data-id="' + row.InvoiceId + '" style=\"\"  role="button" data-number="' + row.Code + '" target="_blank" href="/Modules/Invoices/Create.aspx?Mode=Edit&InvoiceId=' + row.InvoiceId + '">แก้ไข</a></li>';

                        content += '<li><a data-id="' + row.InvoiceId + '" style=\"\" data-number="' + row.Code + '" target="_blank" href="/Modules/Invoices/Create.aspx?InvoiceId=' + row.InvoiceId + '">ข้อมูล</a></li>';
                        ...
                        return "<div class='d-inline-block'>" + content + btnPayment + "</div>";
                    }
                },
        ...
        $("#btnAdvSearch").bind('click', function (e) {
            ...
            switch ($("#ul-table li a.active").data('table')) {
                case "allSearch":
    ...
    return {
        ...
        init: function () {
            ...
            $("#ul-table li a.active").data('timestamp', _currentTimestamp);
...
var fnInvoicePaymentModal = function () {
    ...
    var initHandles = function () {
        ...
        $("#btnVoid").click(function () {
            let _rowTable = null;
            let _invoiceItem = [];
            if ($("#allSearch").is(":visible")) {
                _rowTable = $("#allSearch tbody tr");
            } else if ($("#waitingSearch").is(":visible")) {
                _rowTable = $("#waitingSearch tbody tr");
            } else if ($("#overSearch").is(":visible")) {
                _rowTable = $("#overSearch tbody tr");
            }

            $.each(_rowTable, function (_index, _data) {
                let _checked = $(_data).find("td input[type=checkbox]");

                if (_checked.length > 0) {
                    if ($(_checked[0]).prop("checked")) {
                        let _invoice_id = parseInt($(_checked[0]).attr("data-invoice-id"));
                        _invoiceItem.push(_invoice_id);
                    }
                }
            });

            console.log(_invoiceItem);
            if (_invoiceItem.length > 0) {
                $.confirm({
                    title: '<h3>แจ้งเตือน</h3>',
                    content: '<h3>ท่านต้องการยกเลิกการแจ้งหนี้หรือไม่</h3>',
                    theme: 'bootstrap',
                    buttons: {
                        "<span style=\"font-size: 20px;\">ตกลง</span>": function () {
                            $.ajax({
                                type: 'POST',
                                url: "/Modules/Invoices/Index.aspx/InvoiceVoid",
                                dataType: "JSON", // data type expected from server
                                data: JSON.stringify({ InvoiceId: _invoiceItem }),
                                contentType: "application/json",
                ...
                $.confirm({
                    title: '<h3>แจ้งเตือน</h3>',
                    content: "<h3>กรุณาเลือกรายการที่ต้องยกเลิกการแจ้งหนี้</h3>",
                    theme: 'bootstrap',
                    buttons: {
                        "<span style=\"font-size: 20px;\">ปิด</span>": function () {
                        }
        ...
        //$("#invoice-payment-form").submit(function (event) {
        //    formValidation();
        //    event.preventDefault();
        //});
        ...
        $('#modal-invoice-payment').on('shown.bs.modal', function (e) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    let lbremainpaymentamount = 0;//parseFloat(result.Result.TotalAmount !== null ? result.Result.TotalAmount.toFixed(2) : 0.00);
                    ...
                    let lbremainpaymentamount_0 = 0;// parseFloat(result.Result.TotalAmount !== null ? result.Result.TotalAmount.toFixed(2) : 0.00);// + lbremainpaymentamount_1;
                    let lbremainpaymentamount_2 = 0;

                    $.each(result.Result.InvoiceItems, function (e, s) {
                        if (s.IsDelete == false) {
                            lbremainpaymentamount_0 += s.Amount;
                        }
                    });

                    lbremainpaymentamount = lbremainpaymentamount_0;

                    $.each(result.Result.PaidPayments, function (e, s) {
                        if (s.Status !== "Void") {
                            lbremainpaymentamount -= parseFloat(s.Amount !== null ? s.Amount.toFixed(2) : 0.00);
                            lbremainpaymentamount_2 += parseFloat(s.Amount !== null ? s.Amount.toFixed(2) : 0.00);
                        }
                    });

                    lbremainpaymentamount -= lbremainpaymentamount_1;

                    //if (result.Result.ManualDiscount !== null) {
                    //    lbremainpaymentamount -= parseFloat(result.Result.ManualDiscount !== null ? result.Result.ManualDiscount.toFixed(2) : 0.00);
                    //}
                    if (lbremainpaymentamount > 0) {
                        $('.PaidAmount').addClass('form-group');
                    } else {
                        $('.PaidAmount').removeClass('form-group');
                    }
                    ...
                    if (result.Result.Fd_NewTermSubLevel2 != null) {
                        $("#classtName").val(result.Result.Fd_NewTermSubLevel + ' / ' + result.Result.Fd_NewTermSubLevel2);
                    } else if (result.Result.Fd_NewTermSubLevel != null) {
                        $("#classtName").val(result.Result.Fd_NewTermSubLevel);
                    } else {
                        $("#classtName").val(result.Result.SubLevel + ' / ' + result.Result.SubLevel2);
                    }
                    ...
                    success: function (resultData, textStatus, xhr) {
                        if (resultData.StatusCode === "200" && resultData.Result !== null) {
                            $.confirm({
                                ...
                                buttons: {
                                    "<span style=\"font-size: 20px;\">ตกลง</span>": function () {
                                        window.open("/Modules/Invoices/PrintBillPreview.aspx?InvoiceId=" + data.InvoiceId + "&paidPaymentId=" + resultData.Result, "_blank");
                                        $("#btnAdvSearch").click();
...
var fnInvoicePaymentMethodModel = function () {
    ...
    var initHandles = function () {
        ...
        $('#btnSavePaymentMethod').on('click', function (e) {
            var numberPaymentmethodForm = $("li.li-paymentmethod:not(.d-none)").data('paymentmethod');
