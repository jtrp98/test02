 ...
 */
 ...
 (function($){
	...
	$.fn.tablePagination = function(settings) {
      ...
      if (table.id) {
        totalPagesId = '#'+table.id+'+#tablePagination #'+table.id+'tablePagination_totalPages';
        currPageId = '#'+table.id+'+#tablePagination #'+table.id+'tablePagination_currPage';
        rowsPerPageId = '#'+table.id+'+#tablePagination #'+table.id+'tablePagination_rowsPerPage';
        firstPageId = '#'+table.id+'+#tablePagination #'+table.id+'tablePagination_firstPage';
        prevPageId = '#'+table.id+'+#tablePagination #'+table.id+'tablePagination_prevPage';
        nextPageId = '#'+table.id+'+#tablePagination #'+table.id+'tablePagination_nextPage';
        lastPageId = '#'+table.id+'+#tablePagination #'+table.id+'tablePagination_lastPage';
      }
      else {
      ...
      function createPaginationElements() {
        var htmlBuffer = [];
        htmlBuffer.push("<div id='tablePagination' class = 'tablePagination'>");
        htmlBuffer.push("<span id='"+table.id+"tablePagination_perPage' class = 'tablePagination_perPage' >");
        htmlBuffer.push(" Page size: ");
        htmlBuffer.push("<select id='"+table.id+"tablePagination_rowsPerPage'><option value='5'>5</option></select>");
        htmlBuffer.push("</span>");
        htmlBuffer.push("<span id='"+table.id+"tablePagination_paginater' class = 'tablePagination_paginater'>");
        htmlBuffer.push("<img id='"+table.id+"tablePagination_firstPage' src='"+defaults.firstArrow+"' onmouseover=\"this.style.cursor='pointer'\" >");
        htmlBuffer.push("<img id='"+table.id+"tablePagination_prevPage' src='"+defaults.prevArrow+"' onmouseover=\"this.style.cursor='pointer'\" >");
        htmlBuffer.push("Page");
        htmlBuffer.push("<input id='"+table.id+"tablePagination_currPage' type='input' value='"+currPageNumber+"' size='1' onkeypress='return false;' onpaste='return false;' onfocus='return false;' onmousedown='return false;' onclick='return false;' onselectstart='return false;' >");
        htmlBuffer.push("of <span id='"+table.id+"tablePagination_totalPages'>"+totalPages+"</span>");
        htmlBuffer.push("<img id='"+table.id+"tablePagination_nextPage' src='"+defaults.nextArrow+"' onmouseover=\"this.style.cursor='pointer'\" >");
        htmlBuffer.push("<img id='"+table.id+"tablePagination_lastPage' src='"+defaults.lastArrow+"' onmouseover=\"this.style.cursor='pointer'\" >");
        htmlBuffer.push("</span>");
        htmlBuffer.push("</div>");
      ...
      if ($(totalPagesId).length == 0) {
        ...
        $('#'+table.id+'tablePagination_currPage').val(currPageNumber);
      }
      resetPerPageValues();
      hideOtherPages(currPageNumber);
