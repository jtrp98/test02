...
var _currentPage = function () {
    ...
    var initPage = function () {
        ...
        if (queryString == '') {
            let _row = $('.white-card').html();
            _row += (`<div class="row" style="padding: 0 10px;">
                <div class="form-group col-md-3">
                    <label class="lb-control content-required">ประเภทกลุ่มชำระ</label>
                    <select class="form-control select2" id="ddl-paymentgroup" name="paymentgroup">
                    </select>
                </div>
                <div class="col-md-2">
                    <label class="lb-control content-required">ตั้งค่าเทอมล่วงหน้า</label><br />
                    <input type="checkbox" id="ckbSetting" />
                </div>
                <div class="col-md-2 ckbSetting" style="display:none;">
                    <label class="lb-control content-required">ชั้นล่วงหน้า</label>
                    <select class="form-control select2" id="ddl-sublevel" name="Fd_NewTermLevel_id">
                    </select>
                </div>
                <div class="col-md-2 ckbSetting" style="display:none;">
                    <label class="lb-control content-required">ห้องล่วงหน้า</label>
                    <select class="form-control select2" id="ddl-sublevel2" name="Fd_NewTermClass_id">
            ...
            $('.white-card').html(_row);

            $.ajax({
                type: 'POST',
                url: "/TuitionFee/Tuitionfeesetting.aspx/getgroup",
                dataType: "JSON", // data type expected from server
                contentType: "application/json",
                success: function (response) {
                    let group_data = $.parseJSON(response.d).data;
                    //console.log(data);
                    $("#ddl-paymentgroup")

                    $("#ddl-paymentgroup option").remove();
                    $("#ddl-paymentgroup").append($("<option></option>")
                        .attr("value", "เลือกลุ่มรายการ")
                        .text("เลือกลุ่มรายการ"));

                    $.each(group_data, function (s, e) {
                        $("#ddl-paymentgroup").append($("<option></option>")
                            .attr("value", group_data[s]['group_id'])
                            .text(group_data[s]['group_name']));
                    });
                },
            ...
            $.ajax({
                type: 'POST',
                url: "/TuitionFee/Tuitionfeesetting.aspx/getclass",
                dataType: "JSON", // data type expected from server
                contentType: "application/json",
                success: function (response) {
                    class_data = $.parseJSON(response.d).data;
                    //console.log(data);
                },
                error: function (error) {
                    console.log('Error: ' + error);
            ...
            $.ajax({
                type: 'POST',
                url: "/TuitionFee/Tuitionfeesetting.aspx/getsublevel",
                dataType: "JSON", // data type expected from server
                contentType: "application/json",
                success: function (response) {
                    let _data = $.parseJSON(response.d).data;
                    //console.log(data);
                    let _control = "#ddl-sublevel";
                    let value_default = "เลือกขั้นปี";
                    let display_value = "sublevel_id";
                    let display_text = "sublevel_name";

                    $(_control + "option").remove();
                    $(_control).append($("<option></option>")
                        .attr("value", value_default)
                        .text(value_default));

                    $.each(_data, function (s, e) {
                        $(_control).append($("<option></option>")
                            .attr("value", _data[s][display_value])
                            .text(_data[s][display_text]));
                    });
                },
                error: function (error) {
                    console.log('Error: ' + error);
            ...
            $("#ddl-sublevel").change(function () {
                let _data = getObjects(class_data, "level_id", $("#ddl-sublevel").val());
                let _control = "#ddl-sublevel2";
                let value_default = "เลือกห้อง";
                let display_value = "class_id";
                let display_text = "class_name";
            ...
            $("#ddl-paymentgroup").change(function () {
                $("#tb-products tbody tr").remove();
                $.ajax({
                    type: 'POST',
                    url: "/TuitionFee/paymentgroup.aspx/getdata",
                    dataType: "JSON", // data type expected from server
                    data: JSON.stringify({ "paymentgroup_id": $("#ddl-paymentgroup").val() }),
                    contentType: "application/json",
                    success: function (response) {
                        let _data = $.parseJSON(response.d).group_list;
                        //console.log(_data);

                        $.each(_data, function (index, item) {
                            appendItem();
                            var lastRow = $("#tb-products tbody tr:last");
                            var newProductOption = new Option(item.payment_name, item.payment_id, false, false);
                            ...
                            //$(lastRow).find(".txt-product-accounting-id").val(item.payment_id);
                            //$(lastRow).find(".txt-product-accounting-name").val(item.payment_name);
                            $(lastRow).find(".txt-product-qty").val(1);
                            $(lastRow).find(".txt-product-price").val(item.price.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
                            $(lastRow).find(".txt-product-discount").val(0);
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    var buttonInvoice = "<div class=\"btn-group\"> \
                        <a  href=\"/Modules/Invoices/PrintBillPreview.aspx?InvoiceId={invoiceId}&paidPaymentId={paidPaymentId}\" target=\"_blank\" class=\"btn btn-primary  btn-lg\">ออกใบเสร็จ</a>\
                                        ...
                                        <span class=\"caret\"></span>\
                                            <span class=\"sr-only\">Toggle Dropdown</span>\
                                                ...
                                                <ul class=\"dropdown-menu\">\
                                                    <li> <a href=\"/Modules/Invoices/PrintBillPreview.aspx?InvoiceId={invoiceId}&paidPaymentId={paidPaymentId}\" target=\"_blank\" style=\"font-size:18px\">ออกใบเสร็จ</a></li>\
    ...
    var formValidation = function () {
        ...
        mainForm.validate({
            ...
            submitHandler: function (form) {
                ...
                if ($('#ckbSetting').prop('checked')) {
                    data["Fd_NewTermLevel_id"] = $("#ddl-sublevel").val();
                    data["Fd_NewTermClass_id"] = $("#ddl-sublevel2").val();
                } else {
                    data["Fd_NewTermLevel_id"] = null;
                    data["Fd_NewTermClass_id"] = null;
                }
