...
namespace FingerprintPayment.App_Logic
{
    ...
    /// </summary>
    public class empList : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            using (JabJaiEntities _db = new JabJaiEntities(Connection.StringConnectionSchool(HttpContext.Current.Session["sEntities"].ToString(), ConnectionDB.Read)))
            {
                ...
                var q_usermaster = _dbMaster.TUsers.Where(w => w.nCompany == tCompany.nCompany && w.cType == "1").ToList();
                ...
                foreach (var data1 in _db.TEmployees.Where(w => w.SchoolID == userData.CompanyID).Where(w => (w.cDel ?? "0") != "1"))
                {

                    var check1 = q_usermaster.Where(w => w.nSystemID == data1.sEmp).FirstOrDefault();
                    if (check1 != null)
                    {
                        //string title = "";
