...
{
    /// <summary>
    /// Summary description for PatternContent
    /// </summary>
    public class PatternContent : IHttpHandler , IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            var Value = fcommon.ReplaceInjection(context.Request["PatternContent"]);
            var PatternContent = fcommon.ReplaceInjection(context.Request.QueryString.AllKeys[0]);
            ...
            var check = db.TStudentCardInfoes.Where(w => w.SchoolID == userData.CompanyID && w.elementName == PatternContent).FirstOrDefault();
            if (check == null)
            {
                card = new TStudentCardInfo();
                card.elementValue = Value;
                card.elementName = PatternContent;
