...
{
    /// <summary>
    /// Summary description for LoadEmpFamilyList
    /// </summary>
    public class LoadEmpFamilyList : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (sortIndex)
            {
                case "1": sortBy = "FamilyRelation"; break;
                case "2": sortBy = "Title"; break;
                case "3": sortBy = "FirstName"; break;
                case "4": sortBy = "LastName"; break;
                case "5": sortBy = "Birthday"; break;
                case "6": sortBy = "PersonalStatus"; break;
            }
            sortBy = string.Format("{0} {1}", sortBy, orderDir.ToUpper());

            string search = Convert.ToString(jsonObject["search"]);

            var json = QueryEngine.LoadEmpFamilyJsonData(draw, pageIndex, pageSize, sortBy, EmployeeGateway.GetUserData().CompanyID, eid, search);
