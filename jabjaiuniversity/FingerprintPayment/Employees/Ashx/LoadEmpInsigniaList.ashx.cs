...
{
    /// <summary>
    /// Summary description for LoadEmpInsigniaList
    /// </summary>
    public class LoadEmpInsigniaList : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (sortIndex)
            {
                ...
                case "2": sortBy = "Grade"; break;
                case "3": sortBy = "Position"; break;
                case "4": sortBy = "Date"; break;
            }
            sortBy = string.Format("{0} {1}", sortBy, orderDir.ToUpper());

            string search = Convert.ToString(jsonObject["search"]);

            var json = QueryEngine.LoadEmpInsigniaJsonData(draw, pageIndex, pageSize, sortBy, EmployeeGateway.GetUserData().CompanyID, eid, search);
