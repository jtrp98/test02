...
{
    /// <summary>
    /// Summary description for LoadEmpTrainingList
    /// </summary>
    public class LoadEmpTrainingList : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (sortIndex)
            {
                case "1": sortBy = "ProjectName"; break;
                case "2": sortBy = "TrainingName"; break;
                case "3": sortBy = "StartDate"; break;
                case "4": sortBy = "EndDate"; break;
            }
            sortBy = string.Format("{0} {1}", sortBy, orderDir.ToUpper());

            string search = Convert.ToString(jsonObject["search"]);

            var json = QueryEngine.LoadEmpTrainingJsonData(draw, pageIndex, pageSize, sortBy, EmployeeGateway.GetUserData().CompanyID, eid, search);
