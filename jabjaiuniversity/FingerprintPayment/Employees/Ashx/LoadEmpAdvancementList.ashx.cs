...
{
    /// <summary>
    /// Summary description for LoadEmpAdvancementList
    /// </summary>
    public class LoadEmpAdvancementList : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (sortIndex)
            {
                case "1": sortBy = "Rank"; break;
                case "2": sortBy = "Job"; break;
                case "3": sortBy = "Department"; break;
                case "4": sortBy = "Province"; break;
                case "5": sortBy = "AdvancementDate"; break;
                case "6": sortBy = "UpdateDate"; break;
            }
            sortBy = string.Format("{0} {1}", sortBy, orderDir.ToUpper());

            string search = Convert.ToString(jsonObject["search"]);

            var json = QueryEngine.LoadEmpAdvancementJsonData(draw, pageIndex, pageSize, sortBy, EmployeeGateway.GetUserData().CompanyID, eid, search);
