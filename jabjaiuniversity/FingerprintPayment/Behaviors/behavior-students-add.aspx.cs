...
{
    public partial class behavior_students_add : BehaviorGateway
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                using (JabJaiEntities dbschool = new JabJaiEntities(Connection.StringConnectionSchool(schoolID,ConnectionDB.Read)))
                {
                    ...
                    DropDownList1.DataSource = dbschool.TBehaviors.Where(w => w.SchoolID == schoolID && w.Status == true && w.Type == 0).ToList();
