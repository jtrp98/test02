<%@ Page Title="" Language="C#" MasterPageFile="~/Material2.Master" AutoEventWireup="true" CodeBehind="NewsForm.aspx.cs" Inherits="FingerprintPayment.Message.NewsForm" %>
                ...
                <div class="card-body">
                    <div>
                        <form id="frmNewsForm" class="form-padding">
                            ...
                            <div class="row send-individual-student" style="display: none;">
                                <div class="col-md-4 ml-auto mr-auto">
                                    <div class="">
                            ...
                            <div class="row">
                                <div class="col-md-10 ml-auto mr-auto">
                                    <div class="">
                                        <label class="col-form-label"><span>LINE : </span></label>
                                        <div class="form-check" style="display: inline-block;">
                                            <label class="form-check-label" style="color: #000;">
                                                <input id="chkSendToLINE" class="form-check-input" type="checkbox" value="" <%=EnableSendLINE %>>
                                                <%= FingerprintPayment.Resources.Resource.ResourceManager.GetString("M401039") %> 
                                                <span class="form-check-sign">
                                                    <span class="check"></span>
                                                </span>
                                            </label>
                                            <span class="fw-bold text-danger">(<%= FingerprintPayment.Resources.Resource.ResourceManager.GetString("M401040") %>)</span>
                                        </div>
                                    </div>
                                    <div class="">
                                        ...
                                        <div class="form-check" style="display: inline-block;">
                                            <label class="form-check-label" style="color: #000;">
                                                <input id="chkSendFileWithOwnerOCR" class="form-check-input" type="checkbox" value="">
                                                OCR
                                                <span class="form-check-sign">
                                                    <span class="check"></span>
                                                </span>
        ...
        function LoadTermSubLevel2(subLevelID, objResult) {
            if (subLevelID) {
                $.ajax({
                    async: false,
                    type: "POST",
                    url: "NewsForm.aspx/LoadTermSubLevel2",
        ...
        $(document).ready(function () {
            ...
            $('#iptUserName').autoComplete({
                ...
                events: {
                    search: function (qry, callback) {
                        ...
                        $.ajax({
                            url: "NewsForm.aspx/GetStudentName",
            ...
            $("#btnSendNews").bind({
                click: function () {
                    ...
                    // Check time setting
                    $("#iptDate").change();
                    var timeDifference = Math.ceil(((Date.parse($("#iptDate").data("DateTimePicker").date().format("MM/DD/YYYY") + ' ' + $('#iptTime').val()) - $.now()) / 1000) / 60);
                    if ($('#sltSendType').val() == '1' && timeDifference < 5) {
                        alert('<%= FingerprintPayment.Resources.Resource.ResourceManager.GetString("M132466") %>');
                        $('#pDateTime').css('color', 'red');
                    ...
                    var groupLevel = [];
                    $('.classroom:checked').each(function () { groupLevel.push($(this).data("id")); });

                    var newsData = {
                        title: $('#iptTitle').val(),
                        messageType: $('#sltMessageType').val(), // 0 = <%= FingerprintPayment.Resources.Resource.ResourceManager.GetString("M401015") %>, 1 = <%= FingerprintPayment.Resources.Resource.ResourceManager.GetString("M401016") %>
                        sendType: $('#sltSendType').val(), // 0 = <%= FingerprintPayment.Resources.Resource.ResourceManager.GetString("M401018") %>, 1 = <%= FingerprintPayment.Resources.Resource.ResourceManager.GetString("M401019") %>(date, time)
                        sendDate: $('#iptDate').val() + ' ' + $('#iptTime').val(),
                        acceptType: $('#sltAcceptType').val(),
                        acceptTypeText: $('#sltAcceptType').find(':selected').text(),
                        groupType: $('input[name=iptGroupType]:checked').val(), // 1 = <%= FingerprintPayment.Resources.Resource.ResourceManager.GetString("M401031") %>, 2 = <%= FingerprintPayment.Resources.Resource.ResourceManager.GetString("M206367") %>
                        group: $('#sltGroup').val(),
                        groupDefault: $('#sltGroup').find(':selected').data('default'),
                        groupLevel: groupLevel,
                        users: [],
                        sendFileWithOwner: $('#chkSendFileWithOwner').is(':checked'),
                        sendFileWithOwnerOCR: $('#chkSendFileWithOwnerOCR').is(':checked'),
                        sendToLINE: $('#chkSendToLINE').is(':checked'),
                        files: [],
                    ...
                    for (var i = 0; i < userData.length; i++) {
                        var data = userData[i];
                        newsData.users.push({ sId: data.id, typeId: data.typeId });
                    }

                    const fileData = newsForm.FileData.filter(v => v.status != "delete");
                    for (var i = 0; i < fileData.length; i++) {
                        var data = fileData[i];
                        newsData.files.push({ contentType: data.contentType, fileName: data.fileName, byteData: data.byteData });
                    }
                    ...
                    $.ajax({
                        type: "POST",
                        url: "NewsForm.aspx/SaveAndSendNews",
            ...
            $('#chkSendFileWithOwnerOCR').click(function () {
                if (!$('#chkSendFileWithOwner').is(':checked') && $('#chkSendFileWithOwnerOCR').is(':checked')) {
                    $('#chkSendFileWithOwner').click();
                }

                $('#chkSendFileWithOwner').prop('disabled', !$('#chkSendFileWithOwner').prop('disabled'));
            });
