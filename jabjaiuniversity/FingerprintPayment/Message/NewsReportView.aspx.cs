...
{
    public partial class NewsReportView : MessageGateway
    {
        protected NewsInfo _NewsInfo = new NewsInfo();
        protected string ReadStatusChart = "";
        protected string MessageReplyChart = "";
        ...
        private void InitialPage()
        {
            ...
            using (JabJaiEntities en = new JabJaiEntities(Connection.StringConnectionSchool(schoolID,ConnectionDB.Read)))
            {
                ...
                switch (smsObj.nActionType)
                {
                    ...
                    default: _NewsInfo.AcceptType = "ไม่มีการตอบรับ"; break;
                }
...
(
	...
	(
        ...
        WHERE mu.SchoolID = {0} AND mu.message_id = {1} AND ISNULL(es.WorkStatus, 1) = 1
	) A
	WHERE IDX=1
    UNION
	SELECT *
...
ORDER BY Code", schoolID, messageBoxObj.nMessageID);
                    ...
                    int readStatus1 = messageUsers.Where(w => w.ReadStatus == "อ่านแล้ว").Count();
                    int readStatus0 = messageUsers.Where(w => w.ReadStatus == "ไม่ได้อ่าน").Count();

                    ReadStatusChart = string.Format(@"// Read Status Chart
            var readStatusData = {{
                labels: ['อ่านแล้ว', 'ยังไม่ได้อ่าน'],
                series: [{0}, {1}]
            }};

            var readStatusOptions = {{
                width: 500,
                height: 300,
                plugins: [
                    Chartist.plugins.legend({{
                        clickable: false,
                        position: 'bottom',
                        legendNames: ['อ่านแล้ว {0} คน', 'ยังไม่ได้อ่าน {1} คน']
                    }})
                ]
            }};

            var readStatusResponsiveOptions = [
                ['screen and (min-width: 640px)', {{
                    chartPadding: 30,
                    labelOffset: 100,
                    labelDirection: 'explode',
                    labelInterpolationFnc: function (value, i) {{
                        var percentage = Math.round(readStatusData.series[i] / readStatusData.series.reduce(sum) * 100) + '%';
                        return readStatusData.labels[i] + ' ' + percentage;
                    }}
                }}],
            ...
            new Chartist.Pie('.ct-chart.read-status', readStatusData, readStatusOptions, readStatusResponsiveOptions);", readStatus1, readStatus0);

                    int[] messageReplyStatus = new int[3] { 0, 0, 0 };

                    string[] messageReply = _NewsInfo.AcceptType.Split('/');
                    for (int i = 0; i < messageReply.Length; i++)
                    {
                        messageReplyStatus[i] = messageUsers.Where(w => w.MessageReply == messageReply[i]).Count();
                    }

                    messageReplyStatus[2] = messageUsers.Where(w => w.MessageReply == "-").Count();

                    MessageReplyChart = string.Format(@"// Message Reply Chart
            var messageReplyData = {{
                labels: ['{0}'{1}, 'ยังไม่ได้ตอบ'],
                series: [{2}{3}, {5}]
            }};

            var messageReplyOptions = {{
                width: 500,
                height: 300,
                plugins: [
                    Chartist.plugins.legend({{
                        clickable: false,
                        position: 'bottom',
                        legendNames: ['{0} {2} คน'{4}, 'ยังไม่ได้ตอบ {5} คน']
                    }})
                ]
            }};

            var messageReplyResponsiveOptions = [
                ['screen and (min-width: 640px)', {{
                    chartPadding: 30,
                    labelOffset: 100,
                    labelDirection: 'explode',
                    labelInterpolationFnc: function (value, i) {{
                        var percentage = Math.round(messageReplyData.series[i] / messageReplyData.series.reduce(sum) * 100) + '%';
                        return messageReplyData.labels[i] + ' ' + percentage;
                    }}
                }}],
                ['screen and (min-width: 1024px)', {{
                    labelOffset: 80,
            ...
            new Chartist.Pie('.ct-chart.message-reply', messageReplyData, messageReplyOptions, messageReplyResponsiveOptions);", messageReply[0], (messageReply.Length == 2 ? string.Format(@", '{0}'", messageReply[1]) : ""), messageReplyStatus[0], (messageReply.Length == 2 ? string.Format(@", {0}", messageReplyStatus[1]) : ""), (messageReply.Length == 2 ? string.Format(@", '{0} {1} คน'", messageReply[1], messageReplyStatus[1]) : ""), messageReplyStatus[2]);
                ...
                // File list
                var newsFileList = en.TNewsFiles.Where(w => w.SchoolID == schoolID && w.nSMS == smsID).ToList();
