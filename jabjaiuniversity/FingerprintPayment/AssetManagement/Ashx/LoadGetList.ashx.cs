...
{
    /// <summary>
    /// Summary description for LoadGetList
    /// </summary>
    public class LoadGetList : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (sortIndex)
            {
                ...
                case "7": sortBy = "Source"; break;
                case "8": sortBy = "Department"; break;
                case "9": sortBy = "Receiver"; break;
            }
            sortBy = string.Format("{0} {1}", sortBy, orderDir.ToUpper());

            string searchCat = Convert.ToString(jsonObject["searchCat"]);
            string searchRec = Convert.ToString(jsonObject["searchRec"]);
            string searchDate = Convert.ToString(jsonObject["searchDate"]);

            var json = QueryEngine.LoadGetJsonData(draw, pageIndex, pageSize, sortBy, searchCat, searchRec, searchDate);
