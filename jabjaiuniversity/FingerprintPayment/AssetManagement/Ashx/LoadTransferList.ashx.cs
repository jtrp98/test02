...
{
    /// <summary>
    /// Summary description for LoadTransferList
    /// </summary>
    public class LoadTransferList : IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (sortIndex)
            {
                ...
                case "6": sortBy = "DepRequest"; break;
                case "7": sortBy = "DepTransfer"; break;
            }
            sortBy = string.Format("{0} {1}", sortBy, orderDir.ToUpper());

            string searchDate = Convert.ToString(jsonObject["searchDate"]);

            var json = QueryEngine.LoadTransferJsonData(draw, pageIndex, pageSize, sortBy, searchDate);
