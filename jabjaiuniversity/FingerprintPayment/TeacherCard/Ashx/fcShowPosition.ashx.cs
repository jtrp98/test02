...
namespace FingerprintPayment.TeacherCard.Ashx
{
    ...
    /// </summary>
    public class fcShowPosition : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            var Value = fcommon.ReplaceInjection(context.Request["ShowPosition"]);
            ...
            using (JabJaiEntities db = new JabJaiEntities(Connection.StringConnectionSchool(userData.CompanyID,ConnectionDB.Read)))
            {
                ...
                var check = db.TTeacherCardInfoes.Where(w => w.SchoolID == userData.CompanyID && w.elementName == ShowPosition).FirstOrDefault();
                if (check == null)
                {
                    card = new TTeacherCardInfo();

                    card.elementValue = Value;
                    card.elementName = ShowPosition;
