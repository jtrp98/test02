...
{
    /// <summary>
    /// Summary description for fcShowIdentity
    /// </summary>
    public class fcShowIdentity : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            var Value = fcommon.ReplaceInjection(context.Request["ShowIdentity"]);
            var ShowIdentity = fcommon.ReplaceInjection(context.Request.QueryString.AllKeys[0]);
            ...
            using (JabJaiEntities db = new JabJaiEntities(Connection.StringConnectionSchool(userData.CompanyID,ConnectionDB.Read)))
            {
                ...
                var check = db.TTeacherCardInfoes.Where(w => w.SchoolID == userData.CompanyID && w.elementName == ShowIdentity).FirstOrDefault();
                if (check == null)
                {
                    card = new TTeacherCardInfo();

                    card.elementValue = Value;
                    card.elementName = ShowIdentity;
