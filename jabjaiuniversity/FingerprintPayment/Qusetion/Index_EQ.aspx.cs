...
{
    public partial class Index_EQ : StudentGateway
    {
        ...
        private void InitialControl()
        {
            ...
            using (JabJaiEntities en = new JabJaiEntities(Connection.StringConnectionSchool(schoolID,ConnectionDB.Read)))
            {
                ...
                var listYear = en.TYears.Where(w => w.SchoolID == schoolID && w.cDel == false).OrderByDescending(o => o.numberYear).ToList();
...
ORDER BY r.sort1, r.sort2", schoolID, slID);
        ...
        public static List<EntityDropdown> LoadTerm(string yearID)
        {
            ...
            if (!string.IsNullOrEmpty(yearID))
            {
                ...
                using (JabJaiEntities _db = new JabJaiEntities(Connection.StringConnectionSchool(schoolID,ConnectionDB.Read)))
                {
                    ...
                    result = _db.TTerms.Where(w => w.SchoolID == schoolID && w.nYear == yID && w.cDel == null)
                        .OrderByDescending(o => o.nTerm)
                        .Select(s => new EntityDropdown
                        {
                            id = s.nTerm,
                            name = s.sTerm
                        }).ToList();
        ...
        public static string[] GetStudentName(string keyword)
        {
            ...
            string query = string.Format(@"
SELECT TOP 14 sName + ' ' + sLastname
FROM TUser
WHERE ISNULL(cDel ,0) != '1' AND SchoolID = {0} 
AND (sName <> '' OR sLastname <> '') AND (sName LIKE N'%{1}%' OR sLastname LIKE N'%{1}%' OR sStudentID LIKE N'%{1}%')
GROUP BY sName, sLastname", schoolID, keyword);

            List<string> result = dbschool.Database.SqlQuery<string>(query).ToList();
