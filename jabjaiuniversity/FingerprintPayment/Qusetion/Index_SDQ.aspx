<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Index_SDQ.aspx.cs" MasterPageFile="~/mp.Master" Inherits="FingerprintPayment.Qusetion.Index_SDQ" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="CPH3" runat="server">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $(".studentList #iptStudentName").autocomplete({
                source: function (request, response) {
                    ...
                    $.ajax({
                        url: "Index_SDQ.aspx/GetStudentName",
        ...
        var studentList = {
            ...
            LoadListData: function () {
                studentList.dt = $(".studentList #tableData").DataTable({
                    ...
                    "ajax": {
                        "type": "POST",
                        "url": "Index_SDQ.aspx/LoadStudent",
                        "contentType": "application/json; charset=utf-8",
                        "data": function (d) {
                    ...
                    "columnDefs": [
                        ...
                        { className: "text-center", "targets": [1, 2, 3, 4, 5, 6, 7] },
                        {
                            "render": function (data, type, row, draw) {
                                var check = row.ValueSDQ;

                                if (check != "0") {
                                    return '<button type="button" class="sucSDQ btn btn-outline-success" style="width: 40%;"> SDQ </button>';
                                } else {
                                    return '<button type="button" class="fSDQ btn btn-outline-secondary" style="width: 40%;"> SDQ </button>';
                                }
                ...
                studentList.dt.on('draw.dt', function () {
                    ...
                    studentList.dt.column(8, { search: 'applied', order: 'applied' }).nodes().each(function (cell, i) {
                        ...
                        $(cell).find(".fSDQ").button().removeClass("ui-widget").attr("Onclick", 'fcSDQ(' + sid + ')');
                        $(cell).find(".sucSDQ").button().removeClass("ui-widget").attr("Onclick", 'successSDQ(' + sid + ')');
                    });
                });
            },
        ...
        function LoadTermSubLevel2(subLevelID, objResult) {
            if (subLevelID) {
                $.ajax({
                    async: false,
                    type: "POST",
                    url: "Index_SDQ.aspx/LoadTermSubLevel2",
        ...
        function fcSDQ(sID) {
            ...
            var link = "Question_SDQ.aspx?sid=" + sID + "&term=" + Term;
            window.location = link;
        }

        function successSDQ(sID) {
            var Term = $(".studentList #sltTerm").children("option:selected").val();

            var link = "Summary_SDQ.aspx?sid=" + sID + "&term=" + Term;
